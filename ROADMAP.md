...
| M1 | Env + analytic differential  | `ExecutionEnv` (Phase-1 dynamics, §4 contract *as amended* — see `ARCHITECTURE.md` §9, 2026-08-04); TWAP + AC schedule wrapped as policies; Monte-Carlo harness; simulated cost mean/variance for both fixed policies match closed forms within pre-stated CI. **The load-bearing correctness milestone** — env bugs die here, not in training curves. | M0 | ☐ **Next** — brief: `docs/briefs/M1-env-and-analytic-differential.md` (task 0: variational optimality certificate; invariant-7 resolution normative; bands pre-stated) |
