...
| M1 | Env + analytic differential  | `ExecutionEnv` (Phase-1 dynamics, §4 contract *as amended* — see `ARCHITECTURE.md` §9, 2026-08-04); TWAP + AC schedule wrapped as policies; Monte-Carlo harness; simulated cost mean/variance for both fixed policies match closed forms within pre-stated CI. **The load-bearing correctness milestone** — env bugs die here, not in training curves. | M0 | ☑ **Done** 2026-08-04 — brief: `docs/briefs/M1-env-and-analytic-differential.md`; deep tier (27 cells × 100 k episodes) green in 2.5 min, no cell using more than 62 % of its band; five exact identities hold to ~1e-12 or better; variational certificate green. Yielded one §9 amendment (Phase 1 is linearised end-to-end; `cost_moments` quarantined to reporting). |
